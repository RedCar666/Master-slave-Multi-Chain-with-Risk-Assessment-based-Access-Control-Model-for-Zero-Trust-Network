...
/**
 * @Auther ChenFei
 * @Date Create On 2022/5/23 16:06
 */
@RestController
public class ManagementController {
    ...
    private final static String uid_prefix = "did:uid:3:";
    ...
    public CommonResponse register(@RequestBody RpcInputBo bo) throws Exception {
        ...
        if (flag) {
            ...
            //返回UID和交易哈希
            return new CommonResponse("202", "followchain2 register success!!!", result);
        }
        //将响应信息转发给对应从链
        return new CommonResponse("404", "followchain2 register false!!!", "verify false!!!");
    }
    ...
    public CommonResponse identify(@RequestBody IdentifyInputBo bo) throws Exception {
        ...
        if (values.length() > 4) {
            ...
            if (flag) {
                RpcOutputBo result = new RpcOutputBo();
                result.setUid(bo.getUid());
                result.setTransactionHash(transactionHash);
                return new CommonResponse("202", "followchain2 identify success!!!", result);
            } else {
                return new CommonResponse("404", "followchain2 identify false!!!", "verify false!!!");
            }
        }
        return new CommonResponse("404", "followchain2 identify false!!!", "transactionResponse is null!!!");
    }
    ...
    public CommonResponse relate(@RequestBody RelateInputBo bo) throws Exception {
        ...
        if (values.length() > 4) {
            ...
            String domain = StrUtil.sub(splitValues.get(4), 1, -1);
            String domainPkg = StrUtil.sub(splitValues.get(5), 1, -1);
            String data = StrUtil.join(",", name, account, pkg,
                    group, domain, domainPkg);
            //验证签名
            boolean flag = SignUtils.verifySignature(bo.getSignInfo(), pkg, data);
            if (flag) {
                ...
                return new CommonResponse("202", "followchain2 relate success!!!", result);
            } else {
                return new CommonResponse("404", "followchain2 relate false!!!", "verify false!!!");
            }
        }
        return new CommonResponse("404", "followchain2 relate false!!!", "transactionResponse is null!!!");
    }
